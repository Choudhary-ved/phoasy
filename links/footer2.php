...
// Add modern button styles to footer2
echo '<style>
    /* Modern Button Styles for Footer2 */
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['g-recaptcha-response'])) {
        ...
        $ultrasafelink7 = $_POST['ultrasafelink'];
...
if (isset($_POST["newwpsafelink7s"])) {
    $ultrasafelink7 = $_POST["newwpsafelink7s"];
    ...
    if (preg_match('/^[a-zA-Z0-9\/\r\n+]*={0,2}$/', $ultrasafelink7)) {
        ...
        if (!filter_var($ultrasafelink, FILTER_VALIDATE_URL)) {
            ...
            error_log("SafeLink: Invalid URL provided in footer2 form submission");
        }
    } else {
        $ultrasafelink = '#'; // Safe fallback
        error_log("SafeLink: Invalid base64 data provided in footer2 form submission");
    }
...
?>
    <center>
        <!-- ADS CODE HERE -->    
        <?php if (!empty($fads5)) echo html_entity_decode($fads5, ENT_QUOTES, 'UTF-8'); ?>
    </center>
    ...
    <center>
        <!-- ADS CODE HERE -->    
        <?php if (!empty($fads6)) echo html_entity_decode($fads6, ENT_QUOTES, 'UTF-8'); ?>
    </center>
    ...
    <center>
        <!-- ADS CODE HERE -->    
        <?php if (!empty($fads7)) echo html_entity_decode($fads7, ENT_QUOTES, 'UTF-8'); ?>
    </center>
        ...
        <div id="hive-snp2" class="hive-text-center" style="display: none;">
            <center>
                <form id="open-continue-form" action="" method="post">
                    ...
                    <input type="hidden" name="ultrasafelink" value="<?php echo $ultrasafelink7; ?>">
                </form>
            </center>
        </div>
    <?php } else { ?>
        <div id="hive-snp2" class="hive-text-center" style="display: none;">
            <center>
                <a href="<?php echo $ultrasafelink; ?>" style="text-decoration: none;">
                    <button id="open-continue-btn" class="hive_btn" style="margin: 0%; text-decoration: none;" rel="noopener nofollow">Open-Continue</button>
                </a>
            </center>
        </div>
    <?php } ?>
    ...
    <center>
        <!-- ADS CODE HERE -->    
        <?php if (!empty($fads8)) echo html_entity_decode($fads8, ENT_QUOTES, 'UTF-8'); ?>
    </center>
    ...
    <center>
        <?php if (!empty($bads)) echo html_entity_decode($bads, ENT_QUOTES, 'UTF-8'); ?>
    </center>
<?php 
}
?>
...
?>
    ...
    function hivelink() {
        ...
        if (robot2) robot2.style.display = "none";
        if (hiveBtn) hiveBtn.style.display = 'block';
